...
async def check_browser_health(context: BrowserContext) -> bool:
    ...
    try:
        ...
        if hasattr(context, 'browser'):
            browser_methods = [method for method in dir(context.browser) if not method.startswith('_')]
...
async def run_browser_task_async(
    ...
    step_callback: Optional[Callable[[Dict[str, Any], Dict[str, Any], int], Awaitable[None]]] = None,
    ...
    store[task_id] = {
        ...
        "expiry_time": (datetime.now() + timedelta(minutes=task_expiry_minutes)).isoformat(),
        "steps": [],
        "result": None,
        "error": None,
    }
    ...
    try:
        ...
        try:
            ...
            if 'step_callback' not in agent_params and 'register_new_step_callback' in agent_params:
                agent_kwargs['register_new_step_callback'] = step_cb
            if 'done_callback' not in agent_params and 'register_done_callback' in agent_params:
                agent_kwargs['register_done_callback'] = done_cb
                
            # Check if all required parameters are set
            missing_params = []
            for param_name, param in agent_params.items():
                if param.default == inspect.Parameter.empty and param_name != 'self' and param_name not in agent_kwargs:
                    missing_params.append(param_name)
            
            if missing_params:
                logger.error(f"Missing required parameters for Agent: {missing_params}")
                raise Exception(f"Missing required parameters for Agent: {missing_params}")
        ...
        if "Browser context is unhealthy" in str(e):
            try:
                logger.info(f"Task {task_id}: Final attempt to reset browser context...")
                ...
                try:
                    ...
                    global browser_context_healthy
                    browser_context_healthy = True
                    logger.info(f"Task {task_id}: Browser context recovered, retrying...")
                    ...
                    try:
                        ...
                        # Check for callbacks
                        if 'step_callback' in inspect.signature(Agent.__init__).parameters:
                            agent_kwargs['step_callback'] = step_cb
                        if 'done_callback' in inspect.signature(Agent.__init__).parameters:
                            agent_kwargs['done_callback'] = done_cb
                        ...
                        logger.error(f"Task {task_id}: Error creating Agent during retry: {str(agent_error)}")
                        raise
                except Exception as retry_error:
                    logger.error(f"Task {task_id}: Retry failed - {str(retry_error)}")
            except Exception as reset_error:
                logger.error(f"Task {task_id}: Final reset attempt failed - {str(reset_error)}")
...
def create_mcp_server(
    ...
    async def call_tool(
        ...
        if name == "mcp__browser_navigate":
            ...
            url = arguments["url"]
            action = arguments.get("action", "Navigate to the given URL and tell me what you see.")
            ...
            try:
                ...
                return [types.TextContent(type="text", text=f"Browser status: {status}")]
            
            except Exception as e:
                logger.error(f"Error checking browser health: {str(e)}")
                return [types.TextContent(type="text", text=f"Error checking browser health: {str(e)}")]
            ...
            try:
                ...
                return [types.TextContent(type="text", text="Browser context reset successfully")]
            
            except Exception as e:
                logger.error(f"Error resetting browser context: {str(e)}")
                return [types.TextContent(type="text", text=f"Error resetting browser context: {str(e)}")]
    ...
    async def read_resource(uri: str) -> list[types.ResourceContents]:
        ...
        if task["status"] == "running":
            ...
            steps_text = "\n".join(
                [f"Step {s['step']}: {s['agent_output'].get('action', 'Unknown action')}" for s in task["steps"]]
            )
            return [
                types.ResourceContents(
                    mimetype="text/plain",
        ...
        if task["result"]:
            ...
            result_text = f"# Browser Task Report\n\n"
